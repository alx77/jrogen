...
public interface RandomArrayStreamer {
    static <T> Stream<T> streamify(List<T> input, int quantity, long maxRangeValue, boolean unique) {
        @SuppressWarnings("unchecked") final T[] a = (T[]) new Object[input.size()];
        input.toArray(a);
        return streamify(a, quantity, maxRangeValue, unique);
    }

    static <T> Stream<T> streamify(T[] input, int quantity, long maxRangeValue, boolean unique) {
        if (unique) {
